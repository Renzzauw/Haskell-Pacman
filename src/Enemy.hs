...
lookForPlayer gs currDir (px, py) enemypos@(ex, ey) firstOp secondOp = case currDir of
      ...
      where upOrDown    | deltaY `firstOp` (-0.05) && checkNewPosition gs DirUp upPos && currDir /= DirDown = (enemypos, DirUp)
                        ...
                        | deltaX `firstOp` (-0.05) && checkNewPosition gs DirLeft leftPos && snd newLeft = (fst newLeft, DirLeft)
                        | deltaX `secondOp` 0.05 && checkNewPosition gs DirRight rightPos && snd newRight = (fst newRight, DirRight)
                        | {-deltaX `secondOp` 1 && -}deltaY `firstOp` (-1) && checkNewPosition gs DirUp upPos == False && checkNewPosition gs DirLeft leftPos && snd newLeft = (fst newLeft, DirLeft)
                        | {-deltaX `firstOp` (-1) && -}deltaY `firstOp` (-1) && checkNewPosition gs DirUp upPos == False && checkNewPosition gs DirRight rightPos && snd newRight = (fst newRight, DirRight)
                        | {-deltaX `secondOp` 1 && -}deltaY `secondOp` 1 && checkNewPosition gs DirDown downPos == False && checkNewPosition gs DirLeft leftPos && snd newLeft = (fst newLeft, DirLeft)
                        | {-deltaX `firstOp` (-1) && -}deltaY `secondOp` 1 && checkNewPosition gs DirDown downPos == False && checkNewPosition gs DirRight rightPos && snd newRight = (fst newRight, DirRight)
            ...
            leftOrRight | deltaX `firstOp` (-0.05) && checkNewPosition gs DirLeft leftPos && currDir /= DirRight = (enemypos, DirLeft)
                        ...
                        | deltaY `firstOp` (-0.05) && checkNewPosition gs DirUp upPos && snd newUp = (fst newUp, DirUp)
                        | deltaY `secondOp` 0.05 && checkNewPosition gs DirDown downPos && snd newDown = (fst newDown, DirDown)
                        | {-deltaY `secondOp` 1 && -}deltaX `firstOp` (-1) && checkNewPosition gs DirLeft leftPos == False && checkNewPosition gs DirUp upPos && snd newUp = (fst newUp, DirUp)
                        | {-deltaY `firstOp` (-1) && -}deltaX `firstOp` (-1) && checkNewPosition gs DirLeft leftPos == False && checkNewPosition gs DirDown downPos && snd newDown = (fst newDown, DirDown)
                        | {-deltaY `secondOp` 1 && -}deltaX `secondOp` 1 && checkNewPosition gs DirRight rightPos == False && checkNewPosition gs DirUp upPos && snd newUp = (fst newUp, DirUp)
                        | {-deltaY `firstOp` (-1) && -}deltaX `secondOp` 1 && checkNewPosition gs DirRight rightPos == False && checkNewPosition gs DirDown downPos && snd newDown = (fst newDown, DirDown)
                        | otherwise = {-error "leftOrRight" -- -} (enemypos, currDir)
            noDir       | True = ((1,1), DirRight)
                        | deltaY `firstOp` (-0.05) && checkNewPosition gs DirUp upPos && snd newUp = (fst newUp, DirUp)
                        | deltaY `secondOp` 0.05 && checkNewPosition gs DirDown downPos && snd newDown = (fst newDown, DirDown)
                        | deltaX `firstOp` (-0.05) && checkNewPosition gs DirLeft leftPos && snd newLeft = (fst newLeft, DirLeft)
                        | deltaX `secondOp` 0.05 && checkNewPosition gs DirRight rightPos && snd newRight = (fst newRight, DirRight)
